...
const ayarlar = require("../replace.js");
...
exports.run = async (client, message, args) => {
  ...
  if (args[0] !== "ayarla" && args[0] !== "sıfırla")
    return message.channel.send(
      "✅ Yanlış Kullanım Lütfen **ayarla** veya **sıfırla** yaz."
    );
  if (args[0] === "sıfırla") {
    if (!sayacsayi) {
  ...
  if (args[0] === "ayarla") {
    ...
    if (args[1] <= message.guild.memberCount) {
      message.channel.send(
        `<a:ayarlar:797756953873874944> | **Sunucudaki kullanıcı sayısından** (${message.guild.memberCount}) **daha yüksek bir değer girmelisin.**`
      );
      return;
    }
    ...
    message.channel.send(
      `<a:tik1:798276995011051530> | **Sayaç** \`${
        args[1]
